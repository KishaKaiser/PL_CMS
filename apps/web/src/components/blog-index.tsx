  ...
  return (
    <PublicSiteShell siteConfig={config}>
      <main className="mx-auto grid max-w-6xl gap-8 p-8 lg:grid-cols-[minmax(0,1fr)_280px]">
        <section>
          <h1 className="mb-2 text-3xl font-bold" style={{ color: config.theme.primaryColor }}>
            {title}
          </h1>
          <p className="mb-4 text-gray-600">{intro}</p>

          <form action={blogPath} className="mb-6 flex gap-3">
            <input
              name="search"
              defaultValue={search}
