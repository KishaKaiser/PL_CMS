...
export default function AdminPluginsPage() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error loading plugins');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleAdd(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body?.message ?? 'Plugin could not be added');
      }
      ...
      setError(err instanceof Error ? err.message : 'Error adding plugin');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function removeModule(id: string) {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Plugin removal failed');
      setModules((current) => current.filter((module) => module.id !== id));
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Error removing plugin');
    }
  }
    ...
    <div className="mx-auto max-w-6xl p-8">
      <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Add and manage installed plugins and platform modules.
          </p>
        </div>
      ...
      <section className="mb-6 rounded-xl border bg-white p-6 shadow-sm">
        ...
        <form onSubmit={handleAdd} className="mt-4 grid gap-4 lg:grid-cols-[1fr_180px_160px]">
          <div>
            <label className="block text-sm font-medium text-gray-700">Plugin name</label>
            <input
              required
              value={name}
      ...
      {loading ? (
        ...
        <p className="text-gray-500">No plugins or modules installed.</p>
      ) : (
