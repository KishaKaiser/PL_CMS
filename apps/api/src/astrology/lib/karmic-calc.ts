...
function findKarmicAspect(angle: number): { type: string; orb: number; isKarmic: boolean } | null {
  ...
  for (const aspect of aspects) {
    ...
    if (diff <= aspect.orb) {
      return { type: aspect.name, orb: diff, isKarmic: aspect.isKarmic };
    }
  }
  return null;
}

// Our chart engine already includes a real 'South Node' point (computed as
// North Node + 180deg), so prefer that over recomputing it here. The fallback
// path exists only for charts generated by some other source that lacks it.
...
export function analyzeKarmicIndicators(chart1: ChartData, chart2: ChartData, aspects: KarmicAspect[]): KarmicIndicator[] {
  ...
  const nodeAspects = aspects.filter((a) => a.person1Planet.includes('Node') || a.person2Planet.includes('Node'));
...
export function determineKarmicConnections(chart1: ChartData, chart2: ChartData, aspects: KarmicAspect[]): KarmicConnection[] {
  ...
  const nodeAspects = aspects.filter(
    (a) => (a.person1Planet.includes('Node') || a.person2Planet.includes('Node')) && a.significance === 'high',
  );
  ...
  const saturnAspects = aspects.filter(
    (a) => (a.person1Planet === 'Saturn' || a.person2Planet === 'Saturn') && a.significance !== 'low',
  );
  ...
  const plutoAspects = aspects.filter((a) => (a.person1Planet === 'Pluto' || a.person2Planet === 'Pluto') && a.significance === 'high');
  ...
  const venusAspects = aspects.filter(
    (a) => (a.person1Planet === 'Venus' || a.person2Planet === 'Venus') && (a.person1Planet.includes('Node') || a.person2Planet.includes('Node')),
  );
...
export function determineRelationshipType(score: number, connections: KarmicConnection[]): string {
  ...
  if (score >= 70) return 'Significant Karmic Relationship';
  if (score >= 50) return 'Moderate Karmic Connection';
  if (score >= 30) return 'Karmic Acquaintance';
